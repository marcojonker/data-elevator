 ...
 * @param callback(error)
 */
ElevatorCmdConstruct.prototype.onRun = function(options, moduleDir, callback) {
    var workingDir = options['working-dir'] ? options['working-dir'] : './data-elevator';
    var moduleDir  = options['module-dir'] ? options['module-dir'] : null;
    ...
    try{
        ...
        var copyFiles = [
            new CopyFile(
                ...
                true),
            new CopyFile(
                path.join(moduleDir, './lib/resources', 'config.js'),
                path.join(workingDir, 'config.js'),
                true),
            new CopyFile(
                path.join(moduleDir, './lib/resources', 'elevator.js'),
