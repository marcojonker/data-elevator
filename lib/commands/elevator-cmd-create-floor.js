...
var FloorFileProvider = require('../data/floor-file-provider.js').FloorFileProvider;
...
ElevatorCmdCreate.prototype.onRun = function(options, callback) {
    ...
    try{
        ...
        FileUtils.createDirectoryPath(config.floorDir);
        
        //Get the next floor file
        var floorFile = FloorFileProvider.getNextFile(config.floorsDir, name);
        
        //Copy the template file to the new migration file
        FileUtils.copyFile(new CopyFile(config.floorTemplateFile, floorFile.filePath, true));
        this.logger.info('>> New floor created: ' + floorFile.filePath);
