...
class ElevatorCmdMove extends ElevatorCmdBase {
  ...
  onRun (options, callback) {
    try {
      ElevatorEngine.move(this.getConfiguration(options), options.floor, this.logger, this.LevelController, (error) => {
        if (error) {
          ...
          if (options.floor === 'top') {
            ...
            this.logger.info('>> Floor ' + options.floor + ' reached')
          }
        }
        return callback(error)
      })
    } catch (error) {
      this.logger.error('>> Failed to move to floor.', error)
      return callback(error)
    }
  }
}
