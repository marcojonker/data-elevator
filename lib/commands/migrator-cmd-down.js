...
MigratorCmdDown.prototype.onRun = function(options, callback) {
    this.logger.info('>> Migrating DOWN');

    MigrationProcessor.migrateDown(this.getConfiguration(options), options['migrate-to'], this.logger, function(error) {
        if(error) {
            self.logger('Failed migrating DOWN: ' + error.message);
        } else {
            self.logger.info('>> Finished migrating DOWN');
        }
