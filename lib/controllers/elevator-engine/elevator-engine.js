...
var _moveElevator = function(config, direction, floors, logger, levelController, callback) {
    ...
    async.eachSeries(floors, function(floor, callback) {
        ...
        if(floorWorker[directionMethod]) {
            async.waterfall([
                function(callback) {
                    floorWorker[directionMethod](parameters, function(error) {
                        if(error) {
                            logger.info('>>>> Elevator stuck at floor ' + floor.identifier + ' ' + floor.name + ': ' + error.message);
                        } else {
                ...
                function(callback) {
                    levelController.saveCurrentLevel(Level.create(floor.identifier, direction), function(error) {
                        //Saving the current floor might go fail but should not break the elevator so it is only logged
                        if(error) {
                            logger.info('>> Faile de store current level for floor ' + floor.identifier);
                        }
            ...
            return callback(Errors.systemError('Invalid floor ' + floor.filePath));
        }
    }, function(error) {
        return callback(error);
    })
            ...
            function(floors, callback) {
                if(floors != null && floors.length > 0) {
                   _moveElevator(config, direction, floors, logger, levelController, function(error) {
                    ...
                    logger.info('>> No need to move the elevator, already on the choosen floor');
                    return callback(null);
                }           
            }
        ], function(error) {
