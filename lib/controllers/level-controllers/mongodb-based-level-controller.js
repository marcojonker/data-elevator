            ...
            collection.findOne(null, function(error, databaseLevel) {
                if(error) {
                    ...
                    collection.save(databaseLevel, null, function(error, result) {
                        if(error) {
                            return callback(Errors.systemError('Failed to update the level in MongoDb database.', error));
                        } else {
                            return callback(null);
                        }
                    });
    ...
    this.getCollection(function(error, database, collection) {
        if(!error) {
            collection.findOne(null, function(error, level) {
                if(error) {
                    return callback(Errors.systemError('Failed to get the level from MongoDb database.', error), null);
                } else {
