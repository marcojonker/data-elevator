 ...
 * Constructor
 */
var MigrationStatusHandlerMongoDb = function(config) {
    this.database = null;
    
    MigrationStatusHandlerMongoDb.super_.apply(this, arguments);
...
util.inherits(MigrationStatusHandlerMongoDb, MigrationStatusHandlerBase);

/**
 * Get open database connection
 * @param callback(error, database)
 */
MigrationStatusHandlerMongoDb.prototype.getConnection = function(callback) {
 ...
 */
MigrationStatusHandlerMongoDb.prototype.getMigrationsCollection = function(callback) {
    var self = this;
 ...
 */
MigrationStatusHandlerMongoDb.prototype.saveStatus = function(identifier, direction, callback) {
    ...
    this.getMigrationsCollection(function(error, database, collection) {
        if(!error) {
            collection.findOne(null, function(error, dbMigrationStatus) {
                if(error) {
                    return callback(Errors.systemError('Failed to get migration status from MongoDb database.', error));
                } else if(!dbMigrationStatus) {
                    collection.insert(migrationStatus, null, function(error, result) {
                        if(error) {
                            return callback(Errors.systemError('Failed to store migration status in MongoDb database.', error));
                        } else {
                    ...
                    collection.save(dbMigrationStatus, null, function(error, result) {
                        if(error) {
                            return callback(Errors.systemError('Failed to update migration status in MongoDb database.', error));
                        } else {
                            return callback(null);
                        }
                    });
 ...
 */
MigrationStatusHandlerMongoDb.prototype.retrieveStatus = function(callback) {
    this.getMigrationsCollection(function(error, database, collection) {
        if(!error) {
            collection.findOne(null, function(error, migrationStatus) {
                if(error) {
                    return callback(Errors.systemError('Failed to get migration status from MongoDb database.', error), null);
                } else {
...
module.exports = MigrationStatusHandlerMongoDb;