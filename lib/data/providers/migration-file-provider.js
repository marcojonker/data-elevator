...
MigrationFileProviderOptions.createFromIdentifiers = function(fromIdentifier, toIdentifier, ascending) {
    if((ascending === true && toIdentifier && fromIdentifier > toIdentifier)  ||
       (ascending === false && fromIdentifier < toIdentifier)) {
        throw new Error('Invalid fromIdentifier and toIdentifier values');
    }
...
MigrationFileProvider.getMigrationFiles = function(directory, options) {    
    ...
    if((options.ascending === false && options.identifierRange.min && options.identifierRange.max) || options.ascending == true) {
        fileNames.forEach(function(fileName) {
            var migrationFile = MigrationFile.fromPath(path.join(directory, fileName));
        
            if(migrationFile instanceof MigrationFile) {
                if((!options.identifierRange.min || options.identifierRange.min <= migrationFile.identifier) &&
                   (!options.identifierRange.max || options.identifierRange.max >= migrationFile.identifier)) {
