...
const CommandLineDirector = require('command-line-director').CommandLineDirector;
const CommandLine = require('command-line-director').CommandLine;
const CommandLineArgumentFactory = require('command-line-director').CommandLineArgumentFactory;
...
class ElevatorBase {
  ...
  getCommandLineDirector() {
    if (!this.commandLineDirector) {
      ...
      const defaultArguments = [
        this.argumentFactory.keyStringValueArgument('configDir', 'Data elevator config dir (./data-elevator)', false, '--config-dir', '-c', null),
        this.argumentFactory.keyStringValueArgument('workingDir', 'Data elevator working dir (./data-elevator)', false, '--working-dir', '-w', defaultWorkingDir),
        this.argumentFactory.flagArgument('verbose', 'Verbose output', '--verbose', '-v')
      ]

      const commandLines = [
        new CommandLine('help', 'Help', 'Display command line help',
          [this.argumentFactory.stringValueArgument('command', 'Command', true, ['help', '?'])].concat(defaultArguments)),
        new CommandLine('construct', 'Construct', 'Construct a new data elevator for the current project',
          [this.argumentFactory.stringValueArgument('command', 'Command', true, ['construct'])].concat(defaultArguments)),
        new CommandLine('add', 'Add', 'Create a new floor (a floor is a layer of migrations)',
          [
            this.argumentFactory.stringValueArgument('command', 'Command', true, ['add']),
            this.argumentFactory.stringValueArgument('name', 'Name of the migration', false)
          ].concat(defaultArguments)),
        new CommandLine('move', 'Move', 'Move the elevator to top or ground floor',
          [
            this.argumentFactory.stringValueArgument('command', 'Command', true, ['move']),
            this.argumentFactory.stringValueArgument('floor', 'Top or ground floor', true, ['ground', 'top'])
          ].concat(defaultArguments)),
        new CommandLine('move', 'Move', 'Move specific amount of floors',
          [
            this.argumentFactory.stringValueArgument('command', 'Command', true, ['move']),
            this.argumentFactory.stringValueArgument('floor', 'Floor', true, ['up', 'down']),
            this.argumentFactory.stringValueArgument('count', 'Number of floors', false, null, /^\d+$/)
          ].concat(defaultArguments)),
        new CommandLine('move', 'Move', 'Move the elevator to a specific floor',
          [
            this.argumentFactory.stringValueArgument('command', 'Command', true, ['move']),
            this.argumentFactory.stringValueArgument('floor', 'Level number to move to', true, null, /^\d+$/)
          ].concat(defaultArguments)),
        new CommandLine('status', 'Status', 'Display current status of the elevator',
          [
            this.argumentFactory.stringValueArgument('command', 'Command', true, ['status'])
          ].concat(defaultArguments)),
        new CommandLine('list', 'List', 'Display a list of all floors',
          [
            this.argumentFactory.stringValueArgument('command', 'Command', true, ['list'])
          ].concat(defaultArguments))
      ]
  ...
  runCommand(command, callback) {
    if (command.command === 'help') {
      console.log(this.getCommandLineDirector().generateHelp())
      callback(null)
    } else {
      ...
      try {
        CommandHandler = require('../commands/elevator-cmd-' + command.command + '.js')
      } catch (error) {
        console.error('Command not found ' + command.command + ' with error ' + error)
      }
   ...
   */
  run(callback) {
    const command = this.getCommandLineDirector().parse()
    if (command) {
