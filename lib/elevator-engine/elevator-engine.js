...
class ElevatorEngine {
   ...
   */
  static _getFloors (config, level, floor, callback) {
    let floors = []
    const fromIdentifier = level ? level.identifier : null
    const options = FloorSelectionOptions.createFromIdentifiers(fromIdentifier, floor)
   ...
   * @param callback(error)
   */
  static move (config, floor, logger, LevelController, callback) {
    // Validate arguments
    if (!config) {
    ...
    async.waterfall([
      ...
      function (level, callback) {
        ElevatorEngine._getFloors(config, level, floor, function (error, floors, options) {
  ...
  static printList (config, logger, LevelController, callback) {
    ...
    levelController.retrieveCurrentLevel(function (error, level) {
      if (!error) {
        ...
        ElevatorEngine._getFloors(config, null, null, function (error, floors, options) {
          if (!error) {
            if (floors) {
