...
var _createFloorSelectOptions = function(level, direction, identifier) {
    ...
    if(toIdentifier !== null && toIdentifier <= 0) {
        throw new Errors.generalError("Levels lower or equal to '0' are not allowed.");       
    };
    ...
    if(level) {
        ...
        if(direction === ElevatorDirection.UP && toIdentifier != null && toIdentifier < fromIdentifier) {
            throw new Errors.generalError("Current level  '" + level.identifier + "' higher or equal to selected floor.");       
        };

        if(direction === ElevatorDirection.DOWN && toIdentifier != null && toIdentifier > fromIdentifier) {
            throw new Errors.generalError("Current level '" + level.identifier + "' lower or equal to selected floor.");       
        };
    } else {
...
var _getFloors = function(config, direction, level, endIdentifier, callback) {
    ...
    var options = null;
    try {
        options = _createFloorSelectOptions(level, direction, endIdentifier);
    } catch(error) {
                ...
                function(callback) {
                    floorWorker[directionMethod](parameters, function(error) {
                        if(error) {
                            ...
                            logger.info('>>>> Migrated ' + direction + ' to floor ' + floor.getLongName()  + '.');
                        }
                        return callback(error);
                    });                
                },
...
ElevatorEngine.move = function(config, direction, endIdentifier, logger, LevelController, callback) {
    ...
    if(direction === ElevatorDirection.DOWN && (endIdentifier === undefined || endIdentifier === null)) {
        return callback(Errors.invalidArgument('endIdentifier', 'integer'));
    }
...
ElevatorEngine.printList = function(config, logger, LevelController, callback) {
    ...
    _getFloors(config, ElevatorDirection.UP, null, null, function(error, floors) {
        if(!error) {
            if(floors) {
