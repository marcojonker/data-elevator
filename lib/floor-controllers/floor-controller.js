...
class FloorSelectionOptions {
   ...
   */
  static createFromIdentifiers (fromIdentifier, toIdentifier, count) {
    ...
    } else if (toIdentifier === FloorIdentifiers.UP) {
      options.identifierRange.min = fromIdentifier
      options.identifierRange.max = fromIdentifier + count
      options.ascending = true
    } else if (toIdentifier === FloorIdentifiers.DOWN) {
      options.identifierRange.min = Math.max(fromIdentifier - count, 0)
      options.identifierRange.max = fromIdentifier
      options.ascending = false
    } else {
