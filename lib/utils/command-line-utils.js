 ...
 * @throws Error
 */
CommandLine.prototype.validatePropertyDataTypes = function(command) {
    if(!command) {
    ...
    this.commandLineArguments.forEach(function(commandLineArgument) {
        ...
        if(value !== null) {
            ...
            if(value !== null) {
                command[commandLineArgument.propertyName] = value;
            } else {
                result = false;
...
CommandLine.prototype.commandFromLookup = function(argumentLookup) {
    ...
    return (found === true && this.validatePropertyDataTypes(command)) ? command : null;
} 
