 ...
 * @result string
 */
const _removeQuotes = function (text) {
 ...
 * @result object
 */
const _createArgumentLookupFromProcessArguments = function () {
  ...
  cmdArguments.forEach(function (cmdArgument) {
    ...
    if (typeof keyValue[0] === 'string' && keyValue[0].charAt(0) === '-') {
      const value = keyValue.length === 2 ? _removeQuotes(keyValue[1]) : true
...
const _parseDataType = function (value, dataType) {
 ...
 */
const CommandLineArgument = function (propertyName, required, argumentName, alias, dataType, type, defaultValue, allowedValues, regularExpression) {
  if (!propertyName) {
 ...
 * @thows Error
 */
const CommandLine = function (commandLineArguments, defaultArguments) {
 ...
 */
const CommandLineParser = function (commandLines) {
...
CommandLineParser.prototype.parse = function () {
  const lookup = _createArgumentLookupFromProcessArguments()
  let command = null
