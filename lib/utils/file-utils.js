...
FileUtils.createDirectoryPath = function(directoryPath, callback) {
  if(typeof directoryPath == 'string' || directoryPath instanceof String) {
    ...
    return callback(Errors.invalidArgument('directoryPath', 'string'));
  }
}
...
FileUtils.createDirectory = function(createDirectory, callback) {
  if(createDirectory instanceof CreateDirectory) {
      ...
      if (error && error.code === 'ENOENT') {
        fs.mkdir(createDirectory.directory, function(error) {
          if(!error) {
            ...
            return callback(Errors.systemError('Failed to create directory: ' + createDirectory.directory, error));
          }
        });
      } else {
          if(createDirectory.failIfExists === true) {
              return callback(Errors.systemError('Directory already exists: ' + createDirectory.directory));
          } else {
              return callback(null);
          }
      }
    ...
    return callback(Errors.invalidArgument('createDirectory', 'CreateDirectory'));
  }
};
    ...
    fs.stat(copyFile.destination, function(error, stats) {
      if (copyFile.failIfExists === false || (error && error.code === 'ENOENT')) {
        try{
          ...
          return callback(Errors.systemError('Failed to copy file: ' + copyFile.source, error));
        }
      } else {
        return callback(Errors.systemError('File already exists: ' + copyFile.destination, error));
      }
    });
  } else {
